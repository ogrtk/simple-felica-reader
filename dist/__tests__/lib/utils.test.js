import { arrayToHex, binArrayToHex, dataViewToUint8Array, hexStringToByteArray, hexToAscii, sleep, } from "@/src/lib/utils"; // 実際のモジュールパスに置き換えてください
...
describe("hexToAscii", () => {
    ...
    it("should throw error for hex string with odd length", () => {
        ...
        expect(() => hexToAscii(input)).toThrow("Invalid hex string: length must be even.");
    });
...
describe("hexStringToByteArray", () => {
    ...
    it("should throw error for hex string with odd length", () => {
        ...
        expect(() => hexStringToByteArray("48656C6C6")).toThrow("Invalid input. The string must be hexadecimal characters.");
    });
    it("should throw error for hex string with invalid characters", () => {
        expect(() => hexStringToByteArray("ZZ")).toThrow("Invalid input. The string must be hexadecimal characters.");
    });
});
