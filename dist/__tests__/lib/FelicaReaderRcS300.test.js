...
function createFakeUSBDevice(simulatedResponse = []) {
    return {
        // USBDevice の各メソッドは非同期処理を模倣
        open: vi.fn(async () => { }),
        opened: true,
        selectConfiguration: vi.fn(async (confValue) => { }),
        claimInterface: vi.fn(async (interfaceNum) => { }),
        releaseInterface: vi.fn(async (interfaceNum) => { }),
        close: vi.fn(async () => { }),
        transferOut: vi.fn(async (endpoint, data) => ({
            bytesWritten: data.length,
            status: "ok",
        })),
        transferIn: vi.fn(async (endpoint, length) => ({
...
describe("FelicaReaderRcS300", () => {
    ...
    rcs300Reader["wrapCTXIns"] = vi.fn(rcs300Reader["wrapCTXIns"].bind(rcs300Reader));
    beforeEach(() => {
        vi.clearAllMocks();
    });
    describe("openDevice", () => {
        test("デバイスをオープンし、初期化コマンドを実行", async () => {
            ...
            expect(mockUSBDevice.selectConfiguration).toHaveBeenCalledWith(mockUSBDevice.configuration?.configurationValue);
            expect(mockUSBDevice.claimInterface).toHaveBeenCalledWith(mockUSBDevice.configuration?.interfaces[1].interfaceNumber);
    ...
    describe("closeDevice", () => {
        test("デバイスをクローズし、リソースを開放", async () => {
            ...
            /** assert */
            expect(mockUSBDevice.transferOut).toHaveBeenCalled();
            expect(mockUSBDevice.releaseInterface).toHaveBeenCalledWith(mockUSBDevice.configuration?.interfaces[1].interfaceNumber);
    ...
    describe("getUsbConfigSet（異常系のみ）", () => {
        test("configuration が取得できない場合に例外発生", () => {
            ...
            /** action & assert */
            expect(() => new FelicaReaderRcS300(fakeDeviceNoConf, false)).toThrow("configurationがありません");
        });
        test("input endpointが取得できない場合に例外発生", () => {
            ...
            const fakeDeviceNoIn = {
                ...
                configuration: {
                    ...
                    interfaces: mockUSBDevice.configuration?.interfaces.map((iface, index) => {
                        ...
                        if (index === mockUSBDevice.configuration?.configurationValue) {
                            return {
                                ...
                                alternate: {
                                    ...iface.alternate,
                                    endpoints: iface.alternate.endpoints.filter((ep) => ep.direction !== "in"),
                                },
                            };
                        }
                        return iface;
            ...
            /** action & assert */
            expect(() => new FelicaReaderRcS300(fakeDeviceNoIn, false)).toThrow("入力USBエンドポイントが取得できませんでした");
        });
        test("output endpointが取得できない場合に例外発生", () => {
                    ...
                    interfaces: mockUSBDevice.configuration?.interfaces.map((iface, index) => {
                        if (index === mockUSBDevice.configuration?.configurationValue) {
                            return {
                                ...
                                alternate: {
                                    ...iface.alternate,
                                    endpoints: iface.alternate.endpoints.filter((ep) => ep.direction !== "out"),
                                },
                            };
                        }
                        return iface;
            ...
            /** action & assert */
            expect(() => new FelicaReaderRcS300(fakeDeviceNoOut, false)).toThrow("出力USBエンドポイントが取得できませんでした");
        });
    });
    describe("wrapCTXIns", () => {
        test("正常系", async () => {
    ...
    describe("operateFeliCa", () => {
        test("正常系：各privateメソッドの実行(wrapCTXIns, sendUsb, recvUsb, unwrapCTXResponse)", async () => {
            ...
            rcs300Reader["wrapCTXIns"].mockResolvedValueOnce(new Uint8Array([0x01, 0x02, 0x03]));
            // モック: sendUsb は何もしない
            rcs300Reader["sendUsb"].mockResolvedValueOnce(undefined);
            ...
            rcs300Reader["recvUsb"].mockResolvedValueOnce({ data: fakeDV });
            /** action */
            const result = await rcs300Reader.operateFelica([0x0a, 0x0b], "TestOperation");
            /** assert */
            expect(result).toEqual({
                length: 4,
                responseCode: 0x05,
    ...
    describe("デバッグモード", () => {
        test("console.logが出力される", async () => {
            ...
            readerDebugMode["recvUsb"] = vi.fn(readerDebugMode["recvUsb"].bind(readerDebugMode));
            readerDebugMode["sendUsb"] = vi.fn(readerDebugMode["sendUsb"].bind(readerDebugMode));
            /** action */
