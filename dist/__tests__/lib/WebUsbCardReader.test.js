...
describe("WebUsbCardReader", () => {
    ...
    describe("connect", () => {
        test("FelicaService が利用可能な場合、インスタンスを返す", async () => {
            ...
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
        ...
        test("FelicaService が利用できない場合、undefined を返す", async () => {
            FelicaService.connectFelicaReader.mockResolvedValue(undefined);
            const reader = await WebUsbCardReader.connect([], false);
    ...
    describe("polling", () => {
        test("正常に polling 応答が得られる場合、その応答を返す", async () => {
            ...
            fakeFelicaService.polling.mockImplementation(() => {
                callCount++;
                return callCount === 2
        ...
        test("エラー発生時、例外を投げ、デバイスがクローズされる", async () => {
            fakeFelicaService.polling.mockRejectedValue(new Error("Polling error"));
            // デバイスが開いている状態をシミュレーション
            fakeFelicaService.felicaReader = { isOpened: true };
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
            const reader = await WebUsbCardReader.connect([], false);
            if (!reader)
                throw new Error("Reader is undefined");
    ...
    describe("requestService", () => {
        test("正常な polling 応答が得られる場合、サービス結果を返す", async () => {
            fakeFelicaService.polling.mockResolvedValue({
                idm: "sampleIdm",
            });
            fakeFelicaService.requestService.mockResolvedValue({
                service: "result",
            });
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
            const reader = await WebUsbCardReader.connect([], false);
            ...
            expect(fakeFelicaService.closeDevice).toHaveBeenCalled();
            expect(fakeFelicaService.requestService).toHaveBeenCalledWith("sampleIdm", [1, 2, 3]);
        });
        test("polling が undefined を返す場合、undefined を返す", async () => {
            fakeFelicaService.polling.mockResolvedValue(undefined);
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
            const reader = await WebUsbCardReader.connect([], false);
            if (!reader)
                throw new Error("Reader is undefined");
        ...
        test("エラー発生時、例外を投げ、デバイスがクローズされる", async () => {
            ...
            fakeFelicaService.requestService.mockRejectedValue(new Error("Request error"));
            fakeFelicaService.felicaReader = { isOpened: true };
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
            const reader = await WebUsbCardReader.connect([], false);
            if (!reader)
    ...
    describe("readWithoutEncryption", () => {
        test("正常な polling 応答が得られる場合、読み取り結果を返す", async () => {
            ...
            fakeFelicaService.readWithoutEncryption.mockResolvedValue({
                data: "readData",
            });
            FelicaService.connectFelicaReader.mockResolvedValue(fakeFelicaService);
            const reader = await WebUsbCardReader.connect([], false);
            ...
            expect(fakeFelicaService.openDevice).toHaveBeenCalled();
            expect(fakeFelicaService.closeDevice).toHaveBeenCalled();
            expect(fakeFelicaService.readWithoutEncryption).toHaveBeenCalledWith("sampleIdm", params);
        });
        test("polling が undefined を返す場合、undefined を返す", async () => {
            fakeFelicaService.polling.mockResolvedValue(undefined);
            const params = [
        ...
        test("エラー発生時、例外を投げ、デバイスがクローズされる", async () => {
            ...
            fakeFelicaService.readWithoutEncryption.mockRejectedValue(new Error("Read error"));
            fakeFelicaService.felicaReader = { isOpened: true };
            const params = [
                {
                    serviceCode: "0009",
            ...
            if (!reader)
                throw new Error("Reader is undefined");
            await expect(reader.readWithoutEncryption(params)).rejects.toThrow("Read error");
            expect(fakeFelicaService.closeDevice).toHaveBeenCalled();
        });
    });
});
